...
class CheckInTruckListViewModel(private val application: Application, activity: Activity) : BaseViewModel(application,activity) {
    ...
    fun viewArrivalVehicles(){
        ...
        try {
            CoroutineScope(Dispatchers.IO).launch {
                ResponseDataHandler(repository.getWaitingCheckInVehicles(),viewArrivalVehicles,viewArrivalVehiclesStatus,application).handleData("ViewArrivalVehicle")
            }
        } catch (ex: Exception){
